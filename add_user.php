	 ...
	 //sأ¼nnikuupأ¤ev
        ...
        if(isset($_POST["confirm_password_input"]) and !empty($_POST["confirm_password_input"])){
            if($_POST["confirm_password_input"] != $_POST["password_input"]){
                $confirm_password_error = "Sisestatud salasأµnad on erinevad!";
            }
        } else {
            $confirm_password_error = "Palun sisesta salasأµna kaks korda!";
        }

        //kui kأµik kombes, salvestame uue kasutaja
        if(empty($firstname_error) and empty($surname_error) and empty($birth_month_error) and empty($birth_year_error) 
        and empty($birth_day_error) and empty($birth_date_error) and empty($gender_error) and empty($email_error) and empty($password_error) and empty($confirm_password_error)){
...
<html lang="et">
  <head>
    <meta charset="utf-8">
	<title>Veebirakendused</title>
  </head>
  <body>
	<h1>Minu veebirakendus</h1>
	...
	<form method="POST" action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]);?>">
      ...
      <select name="birth_year_input">
        ...
        -->
        <label for="birth_day_input">Sünnikuupäe    v: </label>
	  <?php
	    //sأ¼nnikuupأ¤ev
	  ...
	  <?php
		...
		echo "</select> \n";
	  ?>
        <label for="birth_year_inout">Sünniaasta: </label>
        <?php
